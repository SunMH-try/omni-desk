...
def meeting_context_package(req: MeetingContextRequest):
    ...
    result = meeting_context.generate(
        meeting_id=req.meeting_id or "meeting_alpha_weekly_next",
        graph=engine.graph,
        indexes=engine.indexes,
        atoms=engine.atoms,
...
def extract_action_items(req: ActionItemsRequest):
    ...
    # 真实模式下 minutes_id 可能为 None，则加载所有 minutes 来源
    if req.minutes_id:
        raws = load_source("minutes", source_id=req.minutes_id,
                           tenant_id=engine.tenant_id, project_id=engine.project_id)
    else:
        raws = load_source("minutes", tenant_id=engine.tenant_id, project_id=engine.project_id)

    if not raws:
        return {"code": 200, "data": {"action_items": [], "missing_field_report": {}, "project_member_map": {}}}
    ...
    for raw in raws:
        parsed = parse(raw)
        src_url = raw.get("url", "")
        sid = raw.get("source_id", req.minutes_id or "minutes")
        items, missing_report, _ = extract(source_id=sid, source_url=src_url, parsed=parsed)
        all_items.extend(items)
        all_missing.update(missing_report)

    return {
        "code": 200,
        "data": {
            "action_items": items_to_dict(all_items),
            "missing_field_report": all_missing,
            "project_member_map": {},
        },
    }
...
def weekly_window_summary(req: WeeklyWindowRequest):
    ...
    week_start = req.week_start or _default_week_start()
    week_end   = req.week_end   or _default_week_end()
